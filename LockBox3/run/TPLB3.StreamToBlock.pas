...
uses SysUtils, Math, TPLB3.StreamUtils, TPLB3.CFB_Block, TPLB3.Constants,
     TPLB3.CFB_8Bit, TPLB3.PointerArithmetic, TPLB3.I18n;
...
TEncryptorDecryptor = class( TInterfacedObject)
  ...
  public
    constructor Start_EncDec(
      ...
      AdvancedOptions2: TSymetricEncryptionOptionSet);
...
TNoncibleDecryptor = class( TEncryptorDecryptor, IStreamDecryptor)
  private
    ...
    FisShortMessage: boolean;
    FOnSetIV: TSetMemStreamProc;
    FRemainingSeedLen: integer;
    FhasCapturedIV   : boolean;
...
//      Key, PlainText, FBlockCipher, FChaining, FAdvancedOptions)
//    end
//
//  else
  if cfNoNounce in FChaining.ChainingFeatures then
    ...
    result := TNoncibleDecryptor.Start_Decrypt(
      Key, PlainText, FBlockCipher, FChaining, FOnSetIV, FAdvancedOptions)
end;
...
//      Key, CipherText, FBlockCipher, FChaining, FAdvancedOptions)
//    end
//
//  else
  if cfNoNounce in FChaining.ChainingFeatures then
    ...
    result := TNoncibleEncryptor.Start_Encrypt(
      Key, CipherText, FBlockCipher, FChaining, FOnSetIV, FAdvancedOptions)
end;
...
constructor TNoNonceEncryptor.Start_Encrypt( Key1: TSymetricKey;
  ...
  const Chaining1: IBlockChainingModel; AdvancedOptions2: TSymetricEncryptionOptionSet);
begin
inherited Start_EncDec( Key1, CipherText1, BlockCipher1, Chaining1, True, AdvancedOptions2);
...
procedure TNoncibleEncryptor.GetIV;
var
  SizeDiff: integer;
begin
  Move( FNonce.Memory^, FIV.Memory^, FIVSeedLen);
  if FBlockLen > FIVSeedLen then
    FillChar( MemStrmOffset( FIV, FIVSeedLen)^, FBlockLen - FIVSeedLen, 0);
  if assigned( FOnSetIV) then
    begin
    FIV.Position := 0;
    FOnSetIV( FIV);
    SizeDiff := FIV.Size - FBlockLen;
    if SizeDiff <> 0 then
      FIV.Size := FBlockLen;
    if SizeDiff < 0 then
      FillChar( MemStrmOffset( FIV, FBlockLen + SizeDiff)^, -SizeDiff, 0);
    FIV.Position := 0
    end
end;
...
if (X > 0) and (FDataCount = 0) and (not FisAddingSalt) then
  ...
  if (FIVSeedLen > 0) and (not (optSuppressIVSeedPrependage in FAdvancedOptions2)) then
  ...
  if FSaltLen > 0 then
    ...
    FisAddingSalt  := True;
    try
      Move( MemStrmOffset( FNonce, FIVSeedLen)^, FNonce.Memory^, FSaltLen);
...
constructor TNoNonceDecryptor.Start_Decrypt(
  Key1: TSymetricKey; PlainText1: TStream;
  const BlockCipher1: IBlockCipher; const Chaining1: IBlockChainingModel; AdvancedOptions2: TSymetricEncryptionOptionSet);
begin
inherited Start_EncDec( Key1, Plaintext1, BlockCipher1, Chaining1, False, AdvancedOptions2);
...
if FSaltLen > 0 then
    ...
    FSaltAbsorber := nil;
FRemainingSeedLen := FIVSeedLen;
if optSuppressIVSeedPrependage in FAdvancedOptions2 then
  FRemainingSeedLen := 0;
FhasCapturedIV    := False;
...
while (X > 0) or (FLenBuffer = FBlockLen) do
  ...
  // 1. Read the IV.
  if FRemainingSeedLen > 0 then
    begin
    Amnt := FRemainingSeedLen;
    if X < Amnt then
      Amnt := X;
    if Amnt > 0 then
      FIV.CopyFrom( Ciphertext, Amnt);
    Dec( X, Amnt);
    Dec( FRemainingSeedLen, Amnt);
    continue;
    end;

  if (FRemainingSeedLen = 0) and (not FhasCapturedIV) then
    begin
    FhasCapturedIV := True;
    GetIV;
    continue
...
procedure TNoncibleDecryptor.GetIV;
var
  OldSize: integer;
  SizeDiff: integer;
begin
  OldSize := FIV.Size;
  if OldSize < FBlockLen then
    begin
    FIV.Size := FBlockLen;
...
constructor TEncryptorDecryptor.Start_EncDec(
  ...
  AdvancedOptions2: TSymetricEncryptionOptionSet);
begin
FAdvancedOptions2 := AdvancedOptions2;
...
procedure TEncryptorDecryptor.GetIV;
begin
  // TODO
end;
